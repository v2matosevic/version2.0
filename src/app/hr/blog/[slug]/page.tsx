...
export async function generateMetadata({ params }: HrBlogPostPageProps): Promise<Metadata> {
  const { slug } = await params
  const post = await getPostBySlug(slug, 'hr')
  if (!post) return { title: 'Članak nije pronađen — Version2' }
...
export default async function HrBlogPostPage({ params }: HrBlogPostPageProps) {
  const { slug } = await params
  const post = await getPostBySlug(slug, 'hr')
  if (!post) notFound()
