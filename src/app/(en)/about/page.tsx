...
import { ANIMATED_STATS, VALUES, TECH_STACK } from './about-data'
...
export default function AboutPage() {
  return (
    <main id="main-content" className="flex-1">
      ...
      <PageHero headline="We're Version2. We write code." minHeight="50vh" />
      ...
      <ScrollReveal>
        <ContentSection background="base">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1fr_0.8fr]">
            ...
            >
              <div className="absolute inset-0" style={{ background: 'linear-gradient(160deg, #1a1714 0%, #252220 35%, #1e1b18 65%, #151311 100%)' }} />
              <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg viewBox=\'0 0 256 256\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cfilter id=\'noise\'%3E%3CfeTurbulence type=\'fractalNoise\' baseFrequency=\'0.9\' numOctaves=\'4\' stitchTiles=\'stitch\'/%3E%3C/filter%3E%3Crect width=\'100%25\' height=\'100%25\' filter=\'url(%23noise)\'/%3E%3C/svg%3E")', backgroundSize: '128px 128px' }} />
              <div className="absolute inset-0 opacity-[0.06]" style={{ background: 'radial-gradient(ellipse at 30% 70%, #991717, transparent 60%)' }} />
              <div className="relative flex h-full flex-col items-center justify-center gap-4 px-8 text-center">
                <p
                  className="font-heading text-foreground/90"
                  style={{ fontSize: 'var(--text-display)', fontWeight: 'var(--font-weight-headline)', lineHeight: 'var(--leading-tight)' } as React.CSSProperties}
                >
                  V2
                </p>
                <div className="h-px w-12 bg-brand-red/40" />
                <p className="text-muted/60 text-sm tracking-widest uppercase">Zadar, Croatia</p>
              </div>
            </div>
          </div>
        </ContentSection>
      ...
      <ScrollReveal>
          ...
          <div className="flex flex-col gap-8">
            {TECH_STACK.map((group) => (
              <div key={group.category}>
                <p className="mb-3 uppercase text-muted font-body" style={{ fontSize: 'var(--text-overline)', fontWeight: 'var(--font-weight-body-semibold)', letterSpacing: 'var(--tracking-overline)' } as React.CSSProperties}>
                  {group.category}
                </p>
        ...
        <ContentSection background="base">
          <h2 className="mb-4 font-heading text-foreground" style={{ fontSize: 'var(--text-h3)', fontWeight: 'var(--font-weight-headline)' } as React.CSSProperties}>
            Where We Are
          </h2>
          <p className="text-muted max-w-xl" style={{ fontSize: 'var(--text-body-lg)', lineHeight: 'var(--leading-body)' } as React.CSSProperties}>
          ...
          <p className="mt-4 text-foreground">Novigradska 21, 23000 Zadar, Croatia</p>
        </ContentSection>
      </ScrollReveal>
