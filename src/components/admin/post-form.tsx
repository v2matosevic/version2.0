    ...
    <div className="max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <input id="slug" name="slug" type="text" value={form.slug} onChange={handleSlugChange} className={INPUT_CLASS} placeholder="post-url-slug" required />
          <p className="text-faint text-[var(--text-small)] mt-1">Auto-generated from title. Edit to customize.</p>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="language" className={LABEL_CLASS}>Language</label>
            <select id="language" name="language" value={form.language} onChange={handleChange} className={INPUT_CLASS}>
              {LANGUAGES.map((lang) => (
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <a href="/admin/posts" className="px-5 py-2.5 text-muted text-[var(--text-small)] font-[500] rounded-lg hover:text-foreground hover:bg-raised transition-colors">
            Cancel
          </a>
        </div>
      </form>
